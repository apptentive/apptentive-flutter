...
@MainActor
public class ApptentiveFlutterPlugin: NSObject, @preconcurrency FlutterApplicationLifeCycleDelegate, @preconcurrency FlutterPlugin {
    ...
    public func application(_ application: UIApplication, didReceiveRemoteNotification userInfo: [AnyHashable : Any], fetchCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void) -> Bool {
        return Apptentive.shared.didReceiveRemoteNotification(userInfo, fetchCompletionHandler: { result in
    ...
    private func handleRegisterCall(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        Task {
            do {
                try await Apptentive.shared.register(with: appCredentials, region: region, environment: environment)
    ...
    private func handleEngageCall(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        Task {
            do {
                result(try await Apptentive.shared.engage(event: .init(name: eventName), from: nil))
            } catch let error {
    ...
    private func handleShowMessageCenter(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        Task {
            do {
                result(try await Apptentive.shared.presentMessageCenter(from: nil))
            } catch let error {
    ...
    private func handleCanShowInteractionCall(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        Task {
            do {
                result(try await Apptentive.shared.canShowInteraction(event: .init(name: eventName)))
            } catch let error {
    ...
    private func handleLoginCall(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        Task {
            do {
                try await Apptentive.shared.logIn(with: token)
                result(true)
            } catch let error {
    ...
    private func handleLogoutCall(_ result: @escaping FlutterResult) {
        Task {
            try await Apptentive.shared.logOut()
            result(true)
        }
    }
    ...
    private func unpackConfiguration(_ configuration: Any?) -> (Apptentive.AppCredentials, String?, URL?, String?)? {
